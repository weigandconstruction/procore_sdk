# NOTE: This file is auto generated by OpenAPI Generator 7.9.0-SNAPSHOT (https://openapi-generator.tech).
# Do not edit this file manually.

defmodule Procore.Model.Timesheet2 do
  @moduledoc """
  Deprecated. Reference status property.
  """

  @derive Jason.Encoder
  defstruct [
    :id,
    :created_at,
    :updated_at,
    :date,
    :number,
    :created_by,
    :name,
    :status,
    :timecard_entries
  ]

  @type t :: %__MODULE__{
          :id => integer() | nil,
          :created_at => DateTime.t() | nil,
          :updated_at => DateTime.t() | nil,
          :date => Date.t() | nil,
          :number => integer() | nil,
          :created_by =>
            Procore.Model.RestV10ProjectsProjectIdWorkLogsGet200ResponseInnerCreatedBy.t() | nil,
          :name => String.t() | nil,
          :status => String.t() | nil,
          :timecard_entries => [Procore.Model.TimesheetEntries2.t()] | nil
        }

  alias Procore.Deserializer

  def decode(value) do
    value
    |> Deserializer.deserialize(:created_at, :datetime, nil)
    |> Deserializer.deserialize(:updated_at, :datetime, nil)
    |> Deserializer.deserialize(:date, :date, nil)
    |> Deserializer.deserialize(
      :created_by,
      :struct,
      Procore.Model.RestV10ProjectsProjectIdWorkLogsGet200ResponseInnerCreatedBy
    )
    |> Deserializer.deserialize(:timecard_entries, :list, Procore.Model.TimesheetEntries2)
  end
end
